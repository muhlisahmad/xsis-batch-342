...
public class Tetragon extends Shape {
  ...
  public double calculateArea() {
    double s = (side1 + side2 + side3 + side4) / 2;
    this.area = Math.sqrt((s - side1) * (s - side2) * (s - side3) * (s - side4) - 0.25 * (side1*side3 + side2*side4 + diagonal1*diagonal2) * (side1*side3 + side2*side4 - diagonal1*diagonal2));
    return this.area;
  }

  @Override
  public double calculateCircumference() {
    this.circumference = side1 + side2 + side3 + side4;
