...
public class VariantController {
  ...
  @PutMapping("/{slug}")
  public ResponseEntity<?> updateVariantBySlug(@PathVariable String slug, @RequestBody VariantRequest VariantReqBody) {
      LinkedHashMap<String, Object> data = new LinkedHashMap<>();
      try {
        Variant variant = variantService.updateVariantBySlug(slug, VariantReqBody);
        if (variant == null) {
          data.put("code", 404);
          data.put("status", "failed");
          data.put("message", "Cannot Find The Given Variant");
